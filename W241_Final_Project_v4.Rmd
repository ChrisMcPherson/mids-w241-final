...
```{r}
# If duplicate, only retain first email sent, sets up for exploration later
...
m6.1 <- lm(outcome~treatment*gender+factor(city)+factor(bedrooms)+price+sqft+professional,data=d)
...
(2) We also did some work Work to find treatment response rate required to reject null. That is, how much does the 
...
sprintf("\nTo attain enough power, i.e. to drive the standard error small enough (all things unchanged), we would need a sample size of %.0f", newn)

# Conversely we can also estimate the required difference in coefficient:

newest <- 2*z[4]
sprintf("\nWe need to see an effect of greater than: %.3f", newest)
